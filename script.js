...
(function() {
    ...
    function GuessPractice() {
        if (document.getElementById("GuessPractice.tickbox").checked) {
            try {
                let iframeDoc = window.frames[0]?.document;
                if (!iframeDoc) return; // Exit if iframe isn't loaded

                // Check if a video element exists in the iframe
                let videoExists = iframeDoc.getElementById("frameAudioControls").style.display == 'none';
                console.log("Video exists: " + videoExists);
                if (videoExists) return; // Exit if a video is found

                // Continue with guessing logic if no video is found
                if (document.getElementById("activity-title").innerText == "Assignment") {
                    if (!document.getElementById("guessassignments").checked) {
                        return; // End the method call to prevent guessing
                    }
                }

                if (["Instruction", "Assignment", "Warm-Up"].includes(document.getElementById("activity-title").innerText)) {
                    try {
                        let options = iframeDoc.getElementsByClassName("answer-choice-button");
                        if (options.length > 0) {
                    ...
                    } catch (TypeError) {} // Handles text-only practices
                }
            } catch (TypeError) {}
        }
    }
